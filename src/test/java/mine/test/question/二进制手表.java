...
public class 二进制手表 {
    ...
    public void run() {
        ...
        list.sort((o1, o2) -> {
            String[] arr1 = o1.replaceAll("\"","").split("\\:");
            String[] arr2 = o2.replaceAll("\"","").split("\\:");
            Integer h1 = Integer.valueOf(arr1[0]);
            Integer h2 = Integer.valueOf(arr2[0]);
            Integer m1 = Integer.valueOf(arr1[1]);
            Integer m2 = Integer.valueOf(arr2[1]);
            if(h1!=h2){
                return h1-h2;
            }
            return m1-m2;
    ...
    public List<String> readBinaryWatch(int num) {
        int[] mArr = {480, 240, 120, 60, 32, 16, 8, 4, 2, 1};
        List<String> result = new ArrayList<>();
        if (num != 0) {
            List<List<Integer>> list = getList(mArr, num - 1);
            for (List<Integer> temp : list) {
                String hStr = "";
                String mStr = "0";
                Integer timeNum = getSum(temp);
                if(timeNum==-1){
                    continue;
                }
                if(timeNum/60>=12){
                ...
                hStr = String.valueOf(timeNum / 60);
                String mStrTemp = String.valueOf(timeNum % 60);
                if (mStrTemp.length() == 1) {
                    mStr += mStrTemp;
                } else {
                    mStr = mStrTemp;
                }
                result.add(hStr + ":" + mStr);
            }
        } else {
            result.add("0:00");
        }
    ...
    private Integer getSum(List<Integer> temp) {
        Integer hSum = 0;
        Integer mSum = 0;
    ...
    private List<List<Integer>> getList(int[] arr, int num) {
        ...
        for (int i = 0; i < arr.length - num; i++) {
            List<List<Integer>> resultTemp = getList(Arrays.copyOfRange(arr, i + 1, arr.length), num - 1);
            for (List<Integer> temp : resultTemp) {
                temp.add(arr[i]);
            }
